                  ...
                  <tr key={u.id}>
                    <td>
                      <div className="d-flex align-items-center gap-2">
                        <img src={u.avatar} alt="" className="sp-user-avatar" />
                        <span className="sp-product-name">{u.name}</span>
                      </div>
                    </td>
                    <td className="sp-email">{u.email}</td>
                    <td><span className={`sp-role-badge ${roleBadgeClass(u.role)}`}>{u.role}</span></td>
                    <td>{u.store} Store</td>
                    <td>{u.joined}</td>
                    <td>
                      <span className={`sp-status ${u.status === 'Active' ? 'active' : 'inactive'}`}>
                        {u.status}
                      </span>
                    </td>
                    <td>
                      <Link to={`/users/${u.id}`} className="sp-view-btn">View</Link>
                    </td>
                  </tr>
                ))}
              </tbody>
